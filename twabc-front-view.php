...
function twabc_frontend($atts){
	...
	if(count($images) > 0){
		ob_start();
		?>
		<div id="twabc_<?php echo $id; ?>" class="carousel slide" <?php if($atts['use_javascript_animation'] == '0'){ echo ' data-ride="carousel"'; } ?> data-interval="<?php echo $atts['interval']; ?>">
			
			<?php // First content - the carousel indicators
			if( count( $images ) > 1 ){ 
			
			if($atts['showindicator'] === 'true') { ?>
            
				<ol class="carousel-indicators">
				<?php foreach ($images as $key => $image) { ?>
					<li data-target="#twabc_<?php echo $id; ?>" data-slide-to="<?php echo $key; ?>" <?php echo $key == 0 ? 'class="active"' : ''; ?>></li>
				<?php } ?>
				</ol>
			<?php }elseif($atts['showindicator'] === 'numbered') { ?>
				...
				$ind++;
				?>
					<li data-target="#twabc_<?php echo $id; ?>" data-slide-to="<?php echo $key; ?>" <?php echo $key == 0 ? 'class="active"' : ''; ?>><?php echo $ind;?></li>
				<?php } 
				
				
				wp_enqueue_style( 'twabc', plugins_url('asset/css/numbered-indicator.css',__FILE__ ), array(), TWABC_VERSION );
